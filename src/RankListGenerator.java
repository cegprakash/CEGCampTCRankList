...
public class RankListGenerator {
	...
	String getContent(List<Submission> submissions){
		...
		answer += "<thead>";
		answer+= "<tr>";
		answer += "<th>";  answer += "Rank"; answer += "</th>";
		answer += "<th>";  answer += "Name"; answer += "</th>";
		for(int i=0;i<Constants.problemNames.length;i++){
			answer += "<th>";  answer += Constants.problemNames[i]; answer += "</th>";
		}
		answer += "<th>";  answer += "Points"; answer += "</th>";
		answer += "<tr/>";		
		answer += "</thead>";
		...
		for(int i=0; i<submissions.size(); i++){
			answer += "<tr>";
			answer += "<td>"; answer += String.valueOf(i+1); answer +="</td>";
			...
			for(int j=0;j<Constants.problemNames.length;j++){
				answer += "<td bgcolor=\""; answer+=submissions.get(i).didSolve(j+1)?Constants.GREEN:Constants.WHITE; answer +="\"></td>";
				points += submissions.get(i).didSolve(j+1)? 3 : 0;
			}
			answer += "<td>"; answer += String.valueOf(points); answer +="</td>";
			answer += "<tr/>";
		}		
		answer += "</tbody>";
		answer += "</table>";
